...
<head>
	...
	<script src="/resources/js/ps/epc/pcrf/callproc/pcrf_af.js"></script>
</head>
<body>
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<div class="mu-vgroup" style="display: none;">
												<div class="mu-checkbox">
													<input id="search_chk_af" type="checkbox" checked="checked">
													<label for="search_chk_af">AF</label>
												</div>
												<div class="mu-checkbox">
													<input id="search_chk_af_type" type="checkbox" checked="checked">
													<label for="search_chk_af_type">TYPE</label>
												</div>
											</div>
										</td>
									</tr>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
						<table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab' >
							<thead>
								<tr id="headerTr">
									<th class="updown sort" id="NE_NAME">PCRF명</th>
										...
										<th class="updown sort" id="AF">AF</th>
							...
							<c:set var ="listSize" value = "${fn:length(afInfoList)}"/>
							<c:choose>
									...
									<c:otherwise>
										<c:forEach var="afInfoList" items="${afInfoList}" >
										<tr height="25px">
											<td>${afInfoList.NE_NAME}</td>
											<td>${afInfoList.C_DATE}</td>
											<td>${afInfoList.WEEK_ID}</td>
											<td>${afInfoList.TEAM_NAME}</td>
											<td>${afInfoList.PKG_VER}</td>
											<td>${afInfoList.AF}</td>
											<td>${afInfoList.TYPE}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${afInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
