...
<head>
	...
	<script src="/resources/js/ps/epc/sgw/callproc/sgw_bd.js"></script>
</head>
<body>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
						<table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab' >
							...
							<c:set var ="listSize" value = "${fn:length(sgwBdInfoList)}"/>
							<c:choose>
									...
									<c:otherwise>
										<c:forEach var="sgwBdInfoList" items="${sgwBdInfoList}" >
										<tr height="25px">
											<td>${sgwBdInfoList.NE_NAME}</td>
											<td>${sgwBdInfoList.C_DATE}</td>
											<td>${sgwBdInfoList.WEEK_ID}</td>
											<td>${sgwBdInfoList.TEAM_NAME}</td>
											<td>${sgwBdInfoList.PKG_VER}</td>
											<td>${sgwBdInfoList.SGW}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${sgwBdInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
