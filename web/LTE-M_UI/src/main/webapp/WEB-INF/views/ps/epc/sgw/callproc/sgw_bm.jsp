...
<head>
	...
	<script src="/resources/js/ps/epc/sgw/callproc/sgw_bm.js"></script>
</head>
<body>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
						<table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab' >
							...
							<c:set var ="listSize" value = "${fn:length(sgwBmInfoList)}"/>
							<c:choose>
									...
									<c:otherwise>
										<c:forEach var="sgwBmInfoList" items="${sgwBmInfoList}" >
										<tr height="25px">
											<td>${sgwBmInfoList.NE_NAME}</td>
											<td>${sgwBmInfoList.C_DATE}</td>
											<td>${sgwBmInfoList.WEEK_ID}</td>
											<td>${sgwBmInfoList.TEAM_NAME}</td>
											<td>${sgwBmInfoList.PKG_VER}</td>
											<td>${sgwBmInfoList.SGW}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${sgwBmInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
