...
<head>
	...
	<script src="/resources/js/ps/epc/hss/s13_intf.js"></script>
</head>
<body>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
                        <table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab'>
							...
							<c:set var ="listSize" value = "${fn:length(s13InfoList)}"/>
							<c:choose>
...
<!-- 									<td colspan="32" class="last regist" align = "center"><b>조회 결과가 없습니다.</b></td> -->
									...
									<c:otherwise>
										<c:forEach var="s13InfoList" items="${s13InfoList}">
										<tr height="25px">
											<td>${s13InfoList.GROUP_NAME}</td>
											<td>${s13InfoList.C_DATE}</td>
											<td>${s13InfoList.WEEK_ID}</td>
											<td>${s13InfoList.TEAM_NAME}</td>
											<td>${s13InfoList.ATTEMPT}</td>
											<td>${s13InfoList.SUCCESS}</td>
											<td>${s13InfoList.FAIL}</td>
											<td>${s13InfoList.UNAB_COM}</td>
											<td>${s13InfoList.UUNKNOWN}</td>
											<td>${s13InfoList.AUTHFAIL}</td>
											<td>${s13InfoList.IDMISMAT}</td>
											<td>${s13InfoList.AVPERROR}</td>
											<td>${s13InfoList.ROAMPERM}</td>
											<td>${s13InfoList.OPER_NOT}</td>
											<td>${s13InfoList.DATAUNAB}</td>
											<td>${s13InfoList.ETC}</td>
										</tr>
										</c:forEach>
