...
<head>
	...
	<script src="/resources/js/ps/epc/mme/callproc/servicerequest.js"></script>
</head>
<body>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
						<table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab' >
							...
							<c:set var ="listSize" value = "${fn:length(srInfoList)}"/>
							<c:choose>
									...
									<c:otherwise>
										<c:forEach var="srInfoList" items="${srInfoList}" >
										<tr height="25px">
											<td>${srInfoList.NE_NAME}</td>
											<td>${srInfoList.C_DATE}</td>
											<td>${srInfoList.WEEK_ID}</td>
											<td>${srInfoList.TEAM_NAME}</td>
											<td>${srInfoList.PKG_VER}</td>
											<td>${srInfoList.DU_ID}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${srInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
