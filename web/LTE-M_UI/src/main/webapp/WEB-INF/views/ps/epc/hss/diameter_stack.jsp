...
<head>
	...
	<script src="/resources/js/ps/epc/hss/diameter_stack.js"></script>
</head>
<body>
	...
	<div class="mu-container">
		<section>
			...
			<div class="mu-row">
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:700px">
                        <table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab'>
							<thead>
								<tr id="headerTr">
                                        ...
                                        <th class="updown sort" id="IPCFAIL">IPCFAIL</th>
                                        <th class="updown sort" id="SENDFAIL">SENDFAIL</th>
                                        <th class="updown sort" id="PROC_ERR">PROC ERR</th>
                                        <th class="updown sort" id="CONN_ERR">CONN ERR</th>
                                        <th class="updown sort" id="TOOBUSY">TOOBUSY</th>
                                        <th class="updown sort" id="MSG_ERR">MSG ERR</th>
                                        <th class="updown sort" id="ETC">ETC</th>
								</tr>
							</thead>
							<tbody id="grid_contents">
							<c:set var ="listSize" value = "${fn:length(diameterInfoList)}"/>
							<c:choose>
...
<!-- 									<td colspan="32" class="last regist" align = "center"><b>조회 결과가 없습니다.</b></td> -->
                                        <c:forEach begin="0" end="${pagingNum}" step="1">
                                                <tr height="25px">
                                                        <c:forEach begin="1" end="14" step="1">
                                                            <td></td>
                                                        </c:forEach>
                                                </tr>
                                        </c:forEach>
									...
									<c:otherwise>
										<c:forEach var="diameterInfoList" items="${diameterInfoList}">
										<tr height="25px">
											<td>${diameterInfoList.GROUP_NAME}</td>
											<td>${diameterInfoList.C_DATE}</td>
											<td>${diameterInfoList.WEEK_ID}</td>
											<td>${diameterInfoList.TEAM_NAME}</td>
											<td>${diameterInfoList.ATTEMPT}</td>
											<td>${diameterInfoList.SUCCESS}</td>
											<td>${diameterInfoList.FAIL}</td>
											<td>${diameterInfoList.IPCFAIL}</td>
											<td>${diameterInfoList.SENDFAIL}</td>
											<td>${diameterInfoList.PROC_ERR}</td>
											<td>${diameterInfoList.CONN_ERR}</td>
											<td>${diameterInfoList.TOOBUSY}</td>
											<td>${diameterInfoList.MSG_ERR}</td>
											<td>${diameterInfoList.ETC}</td>
										</tr>
										</c:forEach>
