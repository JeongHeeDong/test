...
<head>
    ...
    <script src="<c:url value="/resources/js/ps/ran/callproc/rrc/estab.js"><c:param name="version" value="${nowDate}"/></c:url>"></script>
...
<!-- 													<input type="radio" id="r4" name="search_time" value="time"> -->
										...
										<td>
											...
											<!--<input type="checkbox" id="search_chk_ems">EMS_NAME
											<input type="checkbox" id="search_chk_estab">ESTABCAUSE-->
										</td>
									</tr>
								</tbody>
							</table>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:660px">
                        <table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab'>
							...
							<c:set var ="listSize" value = "${fn:length(estabInfoList)}"/>
							<c:choose>
...
<!-- 									<td colspan="32" class="last regist" align = "center"><b>조회 결과가 없습니다.</b></td> -->
									...
									<c:otherwise>
										<c:forEach var="estabInfoList" items="${estabInfoList}">
										<tr height="25px">
											<td>${estabInfoList.LINE_NAME}</td>
											<td>${estabInfoList.NE_NAME}</td>
											<td>${estabInfoList.CNUM}</td>
											<td>${estabInfoList.C_DATE}</td>
											<td>${estabInfoList.WEEK_ID}</td>
											<td>${estabInfoList.TEAM_NAME}</td>
											<td>${estabInfoList.PKG_VER}</td>
											<td>${estabInfoList.ESTABCAUSE}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${estabInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
<%--                                         <c:set var ="listSize" value = "${fn:length(estabInfoList)}"/> --%>
