...
<head>
	...
	<script src="/resources/js/ps/epc/pcrf/callproc/pcrf_pcef.js"></script>
</head>
<body>
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<div class="mu-vgroup" style="display: none;">
												<div class="mu-checkbox">
													<input id="search_chk_pcef" type="checkbox" checked="checked">
													<label for="search_chk_pcef">PCEF</label>
												</div>
												<div class="mu-checkbox">
													<input id="search_chk_pcef_type" type="checkbox" checked="checked">
													<label for="search_chk_pcef_type">TYPE</label>
												</div>
											</div>
										</td>
									</tr>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
						<table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab' >
							<thead>
								<tr id="headerTr">
									<th class="updown sort" id="NE_NAME">PCRF명</th>
										...
										<th class="updown sort" id="PCEF">PCEF</th>
										<th class="updown sort" id="TYPE">TYPE</th>
							...
							<c:set var ="listSize" value = "${fn:length(pcefInfoList)}"/>
							<c:choose>
									...
									<c:otherwise>
										<c:forEach var="pcefInfoList" items="${pcefInfoList}" >
										<tr height="25px">
											<td>${pcefInfoList.NE_NAME}</td>
											<td>${pcefInfoList.C_DATE}</td>
											<td>${pcefInfoList.WEEK_ID}</td>
											<td>${pcefInfoList.TEAM_NAME}</td>
											<td>${pcefInfoList.PKG_VER}</td>
											<td>${pcefInfoList.PCEF}</td>
											<td>${pcefInfoList.TYPE}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${pcefInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
