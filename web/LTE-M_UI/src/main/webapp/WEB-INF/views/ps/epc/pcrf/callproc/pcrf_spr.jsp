...
<head>
	...
	<script src="/resources/js/ps/epc/pcrf/callproc/pcrf_spr.js"></script>
</head>
<body>
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<div class="mu-vgroup" style="display: none;">
												<div class="mu-checkbox">
													<input id="search_chk_spr" type="checkbox" checked="checked">
													<label for="search_chk_spr">SPR</label>
												</div>
												<div class="mu-checkbox">
													<input id="search_chk_spr_type" type="checkbox" checked="checked">
													<label for="search_chk_spr_type">TYPE</label>
												</div>
											</div>
										</td>
									</tr>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
						<table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab' >
							<thead>
								<tr id="headerTr">
									<th class="updown sort" id="NE_NAME">PCRF명</th>
										...
										<th class="updown sort" id="SPR">SPR</th>
							...
							<c:set var ="listSize" value = "${fn:length(sprInfoList)}"/>
							<c:choose>
									...
									<c:otherwise>
										<c:forEach var="sprInfoList" items="${sprInfoList}" >
										<tr height="25px">
											<td>${sprInfoList.NE_NAME}</td>
											<td>${sprInfoList.C_DATE}</td>
											<td>${sprInfoList.WEEK_ID}</td>
											<td>${sprInfoList.TEAM_NAME}</td>
											<td>${sprInfoList.PKG_VER}</td>
											<td>${sprInfoList.SPR}</td>
											<td>${sprInfoList.TYPE}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${sprInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
