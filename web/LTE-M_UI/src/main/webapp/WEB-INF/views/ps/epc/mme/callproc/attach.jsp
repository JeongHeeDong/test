...
<head>
	...
	<script src="/resources/js/ps/epc/mme/callproc/attach.js"></script>
</head>
<body>
...
<!-- 													<input type="radio" id="r4" name="search_time" value="time"> -->
										...
										<td>
											<div class="mu-vgroup" style="display: none;">
												<div class="mu-checkbox">
													<input id="search_chk_du" type="checkbox" checked="checked">
													<label for="search_chk_du">DU</label>
												</div>
											</div>
										</td>
									</tr>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
						<table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab' >
							<thead>
								<tr id="headerTr">
									<th class="updown sort" id="NE_NAME">MME명</th>
										...
										<th class="updown sort" id="DU_ID">DU</th>
							...
							<c:set var ="listSize" value = "${fn:length(attachInfoList)}"/>
							<c:choose>
									...
									<c:otherwise>
										<c:forEach var="attachInfoList" items="${attachInfoList}" >
										<tr height="25px">
											<td>${attachInfoList.NE_NAME}</td>
											<td>${attachInfoList.C_DATE}</td>
											<td>${attachInfoList.WEEK_ID}</td>
											<td>${attachInfoList.TEAM_NAME}</td>
											<td>${attachInfoList.PKG_VER}</td>
											<td>${attachInfoList.DU_ID}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${attachInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
