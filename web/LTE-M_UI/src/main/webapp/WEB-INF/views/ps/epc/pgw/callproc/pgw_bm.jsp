...
<head>
	...
	<script src="/resources/js/ps/epc/pgw/callproc/pgw_bm.js"></script>
</head>
<body>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
						<table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab' >
							...
							<c:set var ="listSize" value = "${fn:length(pgwBmInfoList)}"/>
							<c:choose>
									...
									<c:otherwise>
										<c:forEach var="pgwBmInfoList" items="${pgwBmInfoList}" >
										<tr height="25px">
											<td>${pgwBmInfoList.NE_NAME}</td>
											<td>${pgwBmInfoList.C_DATE}</td>
											<td>${pgwBmInfoList.WEEK_ID}</td>
											<td>${pgwBmInfoList.TEAM_NAME}</td>
											<td>${pgwBmInfoList.PKG_VER}</td>
											<td>${pgwBmInfoList.PGW}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${pgwBmInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
