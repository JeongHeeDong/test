...
<head>
	...
	<script src="/resources/js/ps/epc/pgw/callproc/pgw_bd.js"></script>
</head>
<body>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
						<table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab' >
							...
							<c:set var ="listSize" value = "${fn:length(pgwBdInfoList)}"/>
							<c:choose>
									...
									<c:otherwise>
										<c:forEach var="pgwBdInfoList" items="${pgwBdInfoList}" >
										<tr height="25px">
											<td>${pgwBdInfoList.NE_NAME}</td>
											<td>${pgwBdInfoList.C_DATE}</td>
											<td>${pgwBdInfoList.WEEK_ID}</td>
											<td>${pgwBdInfoList.TEAM_NAME}</td>
											<td>${pgwBdInfoList.PKG_VER}</td>
											<td>${pgwBdInfoList.PGW}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${pgwBdInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
