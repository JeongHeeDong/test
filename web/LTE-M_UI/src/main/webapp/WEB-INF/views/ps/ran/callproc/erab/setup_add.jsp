...
<head>
    ...
    <script src="<c:url value="/resources/js/ps/ran/callproc/erab/setup_add.js"><c:param name="version" value="${nowDate}"/></c:url>"></script>
</head>
<body>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:660px">
                        <table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab'>
							...
							<c:set var ="listSize" value = "${fn:length(erabSetupAddInfoList)}"/>
							<c:choose>
...
<!-- 									<td colspan="32" class="last regist" align = "center"><b>조회 결과가 없습니다.</b></td> -->
									...
									<c:otherwise>
										<c:forEach var="erabSetupAddInfoList" items="${erabSetupAddInfoList}">
										<tr height="25px">
											<td>${erabSetupAddInfoList.LINE_NAME}</td>
											<td>${erabSetupAddInfoList.NE_NAME}</td>
											<td>${erabSetupAddInfoList.CNUM}</td>
											<td>${erabSetupAddInfoList.C_DATE}</td>
											<td>${erabSetupAddInfoList.WEEK_ID}</td>
											<td>${erabSetupAddInfoList.TEAM_NAME}</td>
											<td>${erabSetupAddInfoList.PKG_VER}</td>
											<%-- <td>${erabSetupAddInfoList.QCI}</td> --%>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${erabSetupAddInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
<%--                                         <c:set var ="listSize" value = "${fn:length(erabSetupAddInfoList)}"/> --%>
