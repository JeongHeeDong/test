...
<head>
	...
	<script src="/resources/js/ps/epc/sgw/callproc/sgw_ba.js"></script>
</head>
<body>
...
<!-- 													<input type="radio" id="r4" name="search_time" value="time"> -->
										...
										<td>
											<div class="mu-vgroup"  style="display: none;">
												<div class="mu-checkbox">
													<input id="search_chk_sgw" type="checkbox" checked="checked">
													<label for="search_chk_sgw">SGW</label>
												</div>
											</div>
										</td>
									</tr>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
						<table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab' >
							<thead>
								<tr id="headerTr">
									<th class="updown sort" id="NE_NAME">SGW명</th>
										...
										<th class="updown sort" id="SGW">SGW</th>
							...
							<c:set var ="listSize" value = "${fn:length(sgwBaInfoList)}"/>
							<c:choose>
									...
									<c:otherwise>
										<c:forEach var="sgwBaInfoList" items="${sgwBaInfoList}" >
										<tr height="25px">
											<td>${sgwBaInfoList.NE_NAME}</td>
											<td>${sgwBaInfoList.C_DATE}</td>
											<td>${sgwBaInfoList.WEEK_ID}</td>
											<td>${sgwBaInfoList.TEAM_NAME}</td>
											<td>${sgwBaInfoList.PKG_VER}</td>
											<td>${sgwBaInfoList.SGW}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${sgwBaInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
