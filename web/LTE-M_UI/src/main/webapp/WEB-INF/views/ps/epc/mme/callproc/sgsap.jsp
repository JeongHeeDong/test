...
<head>
	...
	<script src="/resources/js/ps/epc/mme/callproc/sgsap.js"></script>
</head>
<body>
...
<!-- 													<input type="radio" id="r4" name="search_time" value="time"> -->
										...
										<td>
											<div class="mu-vgroup">
												<div class="mu-checkbox">
													<input id="search_chk_msc" type="checkbox">
													<label for="search_chk_msc">MSC</label>
												</div>
											</div>
										</td>
									</tr>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
						<table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab' >
							<thead>
								<tr id="headerTr">
									<th class="updown sort" id="NE_NAME">MME명</th>
										...
										<th class="updown sort" id="MSC">MSC</th>
							...
							<c:set var ="listSize" value = "${fn:length(sgsapInfoList)}"/>
							<c:choose>
									...
									<c:otherwise>
										<c:forEach var="sgsapInfoList" items="${sgsapInfoList}" >
										<tr height="25px">
											<td>${sgsapInfoList.NE_NAME}</td>
											<td>${sgsapInfoList.C_DATE}</td>
											<td>${sgsapInfoList.WEEK_ID}</td>
											<td>${sgsapInfoList.TEAM_NAME}</td>
											<td>${sgsapInfoList.PKG_VER}</td>
											<td>${sgsapInfoList.MSC}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${sgsapInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
