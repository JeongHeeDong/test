...
<head>
	...
	<script src="/resources/js/ps/epc/hss/sp_intf.js"></script>
</head>
<body>
				...
				<div class="mu-col mu-col-12">
					<div class="gridWrap mt10" style="max-height:691px">
                        <table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab'>
							...
							<c:set var ="listSize" value = "${fn:length(spInfoList)}"/>
							<c:choose>
...
<!-- 									<td colspan="32" class="last regist" align = "center"><b>조회 결과가 없습니다.</b></td> -->
									...
									<c:otherwise>
										<c:forEach var="spInfoList" items="${spInfoList}">
										<tr height="25px">
											<td>${spInfoList.GROUP_NAME}</td>
											<td>${spInfoList.C_DATE}</td>
											<td>${spInfoList.WEEK_ID}</td>
											<td>${spInfoList.TEAM_NAME}</td>
											<td>${spInfoList.ATTEMPT}</td>
											<td>${spInfoList.SUCCESS}</td>
											<td>${spInfoList.FAIL}</td>
											<td>${spInfoList.UNAB_COM}</td>
											<td>${spInfoList.UUNKNOWN}</td>
											<td>${spInfoList.AUTHFAIL}</td>
											<td>${spInfoList.IDMISMAT}</td>
											<td>${spInfoList.AVPERROR}</td>
											<td>${spInfoList.ROAMPERM}</td>
											<td>${spInfoList.OPER_NOT}</td>
											<td>${spInfoList.DATAUNAB}</td>
											<td>${spInfoList.ETC}</td>
										</tr>
										</c:forEach>
