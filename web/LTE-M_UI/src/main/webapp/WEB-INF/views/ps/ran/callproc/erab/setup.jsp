...
<head>
    ...
    <script src="<c:url value="/resources/js/ps/ran/callproc/erab/setup.js"><c:param name="version" value="${nowDate}"/></c:url>"></script>
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<div class="mu-vgroup">
												<div class="mu-checkbox">
													<input id="search_chk_estab" type="checkbox">
													<label for="search_chk_estab">ESTABCAUSE</label>
												</div>
											</div>
										</td>
									</tr>
                        ...
                        <table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab'>
							<thead>
								<tr id="headerTr">
										...
										<th class="updown sort" id="ESTABCAUSE">ESTABCAUSE</th>
										<!-- <th class="updown sort" id="QCI">QCI</th> -->
							...
							<c:set var ="listSize" value = "${fn:length(erabSetupInfoList)}"/>
							<c:choose>
...
<!-- 									<td colspan="32" class="last regist" align = "center"><b>조회 결과가 없습니다.</b></td> -->
									...
									<c:otherwise>
										<c:forEach var="erabSetupInfoList" items="${erabSetupInfoList}">
										<tr height="25px">
											<td>${erabSetupInfoList.LINE_NAME}</td>
											<td>${erabSetupInfoList.NE_NAME}</td>
											<td>${erabSetupInfoList.CNUM}</td>
											<td>${erabSetupInfoList.C_DATE}</td>
											<td>${erabSetupInfoList.WEEK_ID}</td>
											<td>${erabSetupInfoList.TEAM_NAME}</td>
											<td>${erabSetupInfoList.PKG_VER}</td>
											<td>${erabSetupInfoList.ESTABCAUSE}</td>
											<%-- <td>${erabSetupInfoList.QCI}</td> --%>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${erabSetupInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
<%--                                         <c:set var ="listSize" value = "${fn:length(erabSetupInfoList)}"/> --%>
