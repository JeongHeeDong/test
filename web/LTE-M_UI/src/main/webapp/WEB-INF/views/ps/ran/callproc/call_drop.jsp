...
<head>
    ...
    <script src="<c:url value="/resources/js/ps/ran/callproc/call_drop.js"><c:param name="version" value="${nowDate}"/></c:url>"></script>
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<div class="mu-vgroup">
												<div class="mu-checkbox">
													<input id="search_chk_cdropcause" type="checkbox">
													<label for="search_chk_cdropcause">CALLDROPESTABCAUSE</label>
												</div>
											</div>
										</td>
									</tr>
                        ...
                        <table class="mu-grid mu-grid-border mu-grid-strip mu-grid-sort2" id='grid_estab'>
							<thead>
								<tr id="headerTr">
										...
										<th class="updown sort" id="CALLDROPESTABCAUSE">CALLDROPESTABCAUSE</th>
							...
							<c:set var ="listSize" value = "${fn:length(callDropInfoList)}"/>
							<c:choose>
...
<!-- 									<td colspan="32" class="last regist" align = "center"><b>조회 결과가 없습니다.</b></td> -->
									...
									<c:otherwise>
										<c:forEach var="callDropInfoList" items="${callDropInfoList}">
										<tr height="25px">
											<td>${callDropInfoList.LINE_NAME}</td>
											<td>${callDropInfoList.NE_NAME}</td>
											<td>${callDropInfoList.CNUM}</td>
											<td>${callDropInfoList.C_DATE}</td>
											<td>${callDropInfoList.WEEK_ID}</td>
											<td>${callDropInfoList.TEAM_NAME}</td>
											<td>${callDropInfoList.PKG_VER}</td>
											<td>${callDropInfoList.CALLDROPESTABCAUSE}</td>
											<c:forEach begin="1" end="${pegCnt}" var="i" varStatus="s">
	                                        	<c:set var="tempName">PEG${i}</c:set>
	                                        	<td>${callDropInfoList[tempName]}</td>
                                        	</c:forEach>
										</tr>
										</c:forEach>
<%--                                         <c:set var ="listSize" value = "${fn:length(callDropInfoList)}"/> --%>
