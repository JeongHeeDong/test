...
<div id="timeAnalysisBg" class="mu-dialog-background" style="display: none;z-index: 10"></div>
<div id="timeAnalysisUp" class="mu-dialog" style="display: none; width: 1600px; height:; left: 42%; top: 25%;z-index: 11">
    <div id="timeAnalysisTitle" class="mu-dialog-head">
    	<h2><span class="title" style="line-height: 200%;">시간별 알람통계 상세리스트</span></h2>
        <button id="timeAnalysisClose" type="button" class="mu-btn mu-btn-icon"><i class="mu-icon-img cancel"></i></button>
    </div>
	<div class="mu-dialog-body">
		<div class="gridScrollT gridWrap mt10">
			<table id="tb_timeAnalysis_detail_header" class="mu-grid mu-grid-border mu-grid-strip tbFix">
				<colgroup>
					<col width="3%">
					<col width="7%">
					<col width="6%">
		...
		<div class="gridWrap mu-scroll-v" style="height: 480px;max-height: 480px;overflow-y: scroll;">
			<table id="tb_timeAnalysis_detail" class="mu-grid mu-grid-border mu-grid-strip tbFix">
	            <colgroup>
					<col width="3%">
					<col width="7%">
					<col width="6%">
	...
	<div class="mu-dialog-foot">
		<button id="btn_timeAnalysisOk" type="button" class="mu-btn mu-pop-btn mu-btn-icon">확인</button>
	</div>
</div>

<script>
function getTimeAnalysisDetailData(condition){
	$.ajax({
		cache : false,
		type : 'post',
		url : '/failureStatistic/analysis/failureAnalysisByTime/getTimeAnalysisDetailData',
		contentType: 'application/json; charset=UTF-8',
		dataType:'json',
		data : JSON.stringify(condition),
		success: function(data) {
			if(data['timeAnalysisDetailData'] != null && data['timeAnalysisDetailData'].length > 0){
				gridTimeAnalysisDetail(data['timeAnalysisDetailData']);
				
			} else {
				$("#tb_timeAnalysis_detail tbody tr").remove();
				$("#timeAnalysisClose").click();
				alert('조회된 데이터가 없습니다.');
			}
		}
	});
...
function gridTimeAnalysisDetail(detailData){
	var searchRow = "";
	var severity = '';
	var alarmStat = '';
	var alarmType = '';
	...
	$("#tb_timeAnalysis_detail tbody").append(searchRow);
