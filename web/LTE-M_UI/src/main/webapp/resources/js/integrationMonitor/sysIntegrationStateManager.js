...
//			if ( lineName_tbl.indexOf(lineId_slt) >= 0 ){
//				$(row).css('display','');
...
function getStateData(){
	...
	$.ajax({
		...
		success: function (data) {
			...
			if ( maxDateTime > vmMaxDateTime){
				$('#maxDateTime').text('감시시간 : '+maxDateTime);
			}else{
				$('#maxDateTime').text('감시시간 : '+vmMaxDateTime);
			}
...
//				}
				...
				var diskLevel = value.DISK_LEVEL;
				if ( value.DISK_LEVEL.indexOf('^') >0 ){
					diskLevel = _.min(value.DISK_LEVEL.split('^'))
				}
				if( Number(filterLevel) >= Number(diskLevel)){
					if(diskLevel == 1){
						criticalCount++;
					}else if(diskLevel == 2){
						majorCount++;
