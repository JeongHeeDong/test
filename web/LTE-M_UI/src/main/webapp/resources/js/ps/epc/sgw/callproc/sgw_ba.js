...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_epc/ps_sgw/ps_sgw_ho/sgw_ba/search',
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.sgwBaInfoLength.length <= 0?0:data.sgwBaInfoLength[0].TOT_CNT;
...
function excel_download(){
		...
		var url =  "/ps/excelDown?PAGE=SGWBA&HEADERS="+headers+"&COLUMNS="+columns+
				...
				"&TITLE=BA_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
