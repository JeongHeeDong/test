...
function configure() {
	...
	if(!$("#search_chk_af_type").is(":checked")){
...
function searchConfig () {
	...
	searchInfo.AF = $("#search_chk_af").is(":checked");
	searchInfo.AF_TYPE = $("#search_chk_af_type").is(":checked");
...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_epc/ps_pcrf/ps_pcrf_af/search',
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.afInfoLength.length <= 0?0:data.afInfoLength[0].TOT_CNT;
...
function excel_download(){
		...
		var url =  "/ps/excelDown?PAGE=AF&HEADERS="+headers+"&COLUMNS="+columns+
				...
				"&TYPE="+searchInfo.TYPE +"&AF="+searchInfo.AF+"&AF_TYPE="+searchInfo.AF_TYPE+
				"&TITLE=AF_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
