...
function changeSortOption(){
	if(searchInfo.TYPE=='sum'){
		if(!$("#search_trend_day").is(":checked") && !$("#search_trend_hour").is(":checked") && !$("#search_system").is(":checked")){
			var key = $("#grid_record th:nth-child(2)").attr('id');
			...
			var key = $("#grid_record th:nth-child(1)").attr('id');
			if ((jQuery.inArray(key,columnSorting.beforeColNms) != -1)){
				var index = columnSorting.beforeColNms.indexOf(key);
				columnSorting.sortInfo.splice(index,1);
			}
...
function searchConfig () {
	searchInfo.SYSTEM_ID = ("'"+$('#search_du_hidden').val().replace(/,/g, "','")+"'")=="''"?'':("'"+$('#search_du_hidden').val().replace(/,/g, "','")+"'");
	searchInfo.T_DAY =$("#search_trend_day").is(":checked");
	searchInfo.T_HOUR = $("#search_trend_hour").is(":checked");
	searchInfo.T_SYSTEM =$("#search_system").is(":checked");
	...
	searchInfo.TYPE = $('#search_record :radio[name="search_cal"]:checked').val();
...
function remakeTd(){
	$("#grid_record th:nth-child(2)").text('일자');

	if(searchInfo.TYPE == 'sum'){
		// 합계
		$("#grid_record td:nth-child(1)").hide();
		$("#grid_record th:nth-child(1)").hide();
		$("#grid_record td:nth-child(2)").hide();
		$("#grid_record th:nth-child(2)").hide();
		
		// 일간추세소통, 시간추세소통 미체크 or 개별시스템별 체크
		if((!$("#search_trend_day").is(":checked") && !$("#search_trend_hour").is(":checked")) || $("#search_system").is(":checked")) {
			$("#grid_record td:nth-child(1)").show();
			$("#grid_record th:nth-child(1)").show();
		}
		// 일간추세소통 체크
		if($("#search_trend_day").is(":checked")) {
			$("#grid_record th:nth-child(2)").show();
			$("#grid_record td:nth-child(2)").show();
		}
		// 시간추세소통 체크
		if($("#search_trend_hour").is(":checked")) {
			$("#grid_record th:nth-child(2)").text('시간');
			$("#grid_record th:nth-child(2)").show();
			$("#grid_record td:nth-child(2)").show();
		}
	} else if(searchInfo.TYPE == 'day' || searchInfo.TYPE == 'hour'){
		...
		if((searchInfo.TYPE == 'day')
			...
			if($("#search_trend_day").is(":checked") || $("#search_trend_hour").is(":checked")) {
				$("#grid_record th:nth-child(2)").text('일시');
			}
			$("#grid_record th:nth-child(2)").show();
			$("#grid_record td:nth-child(2)").show();
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.infoLength.length <= 0?0:data.infoLength[0].TOT_CNT;
...
function excel_download(){
	...
	var columns;
	var i=0;
	$('#grid_record thead tr').each(function(){
	...
	var url = "/ps/excelDown?PAGE=RECORD&HEADERS="+headers+"&COLUMNS="+columns+
			"&SYSTEM_ID="+searchInfo.SYSTEM_ID+"&T_DAY="+searchInfo.T_DAY+"&T_HOUR="+searchInfo.T_HOUR+"&FROM="+searchInfo.FROM+
			"&TO="+searchInfo.TO+"&T_SYSTEM="+searchInfo.T_SYSTEM+"&EXCEPT_HOUR="+searchInfo.EXCEPT_HOUR +"&EXCEPT_DAY="+searchInfo.EXCEPT_DAY +
			"&TYPE="+searchInfo.TYPE +"&TITLE=녹취저장_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
...
$(document).ready(function(){
	...
	$('#search_du_btn').click(function () {
		openDialog('#dlg_du', function (fnCloseDlg) {
			...
			sidArr = _.pluck(rows, 'SYSTEM_NAME');
			sids = sidArr.join(",");
			$('#search_du').val(sids);
			sidArr = _.pluck(rows, 'SYSTEM_ID');
			sids = sidArr.join(",");
	...
	$("#search_trend_day").click(function() {
		//$("#search_system").prop("checked", false);
		$("#search_trend_hour").prop("checked", false);
		if($('#search_record :radio[name="search_cal"]:checked').val() == 'sum') {
