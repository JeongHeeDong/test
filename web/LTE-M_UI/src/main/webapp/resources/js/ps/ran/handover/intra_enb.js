...
function changeSortOption(){
	if(!$("#search_chk_tcid").is(":checked")){
		var key = $("#grid_handover th:nth-child(8)").attr('id');
	...
	if(!$("#search_chk_hocause").is(":checked")){
		var key = $("#grid_handover th:nth-child(9)").attr('id');
		if ((jQuery.inArray(key,columnSorting.beforeColNms) != -1)){
			var index = columnSorting.beforeColNms.indexOf(key);
			columnSorting.sortInfo.splice(index,1);
		}
...
function searchConfig () {
	...
	searchInfo.TYPE = $('#search_handover :radio[name="search_cal"]:checked').val();
	searchInfo.TCID = $("#search_chk_tcid").is(":checked");
	searchInfo.HOCAUSE = $("#search_chk_hocause").is(":checked");
...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_ran/hand_over/intra_enb/search',
...
function remakeTd(){
	$("#grid_handover th:nth-child(4)").text('일자');
	if($("#search_chk_tcid").is(":checked")) {
		$("#grid_handover td:nth-child(1)").show();
		$("#grid_handover th:nth-child(1)").show();
		$("#grid_handover td:nth-child(8)").show();
		$("#grid_handover th:nth-child(8)").show();
	} else {
		$("#grid_handover td:nth-child(8)").hide();
		$("#grid_handover th:nth-child(8)").hide();
	}
	if($("#search_chk_hocause").is(":checked")) {
		$("#grid_handover td:nth-child(1)").show();
		$("#grid_handover th:nth-child(1)").show();
		$("#grid_handover td:nth-child(9)").show();
		$("#grid_handover th:nth-child(9)").show();
	} else {
		$("#grid_handover td:nth-child(9)").hide();
		$("#grid_handover th:nth-child(9)").hide();
	}
	...
	if(searchInfo.TYPE == 'sum'){
		...
		$("#grid_handover td:nth-child(1)").show();
		$("#grid_handover th:nth-child(1)").show();
		$("#grid_handover td:nth-child(2)").hide();
		$("#grid_handover th:nth-child(2)").hide();
		$("#grid_handover td:nth-child(3)").hide();
		$("#grid_handover th:nth-child(3)").hide();
		$("#grid_handover th:nth-child(4)").hide();
		$("#grid_handover td:nth-child(4)").hide();
		$("#grid_handover th:nth-child(5)").hide();
		$("#grid_handover td:nth-child(5)").hide();
		
		// 일간추세소통, 시간추세소통 미체크 or 개별시스템별 체크
		if((!$("#search_trend_day").is(":checked") && !$("#search_trend_hour").is(":checked")) || $("#search_system").is(":checked")) {
			$("#grid_handover td:nth-child(1)").show();
			$("#grid_handover th:nth-child(1)").show();
			$("#grid_handover td:nth-child(2)").show();
			$("#grid_handover th:nth-child(2)").show();
			$("#grid_handover td:nth-child(3)").show();
			$("#grid_handover th:nth-child(3)").show();
		}
		// 일간추세소통 체크
		if($("#search_trend_day").is(":checked")) {
			$("#grid_handover td:nth-child(1)").show();
			$("#grid_handover th:nth-child(1)").show();
			$("#grid_handover th:nth-child(4)").show();
			$("#grid_handover td:nth-child(4)").show();
			$("#grid_handover th:nth-child(5)").show();
			$("#grid_handover td:nth-child(5)").show();
		}
		// 시간추세소통 체크
		if($("#search_trend_hour").is(":checked")) {
			$("#grid_handover td:nth-child(1)").show();
			$("#grid_handover th:nth-child(1)").show();
			$("#grid_handover th:nth-child(4)").text('시간');
			$("#grid_handover th:nth-child(4)").show();
			$("#grid_handover td:nth-child(4)").show();
		}
	} else if(searchInfo.TYPE == 'day' || searchInfo.TYPE == 'hour'){
		...
		if($("#search_system").is(":checked")) {
			...
			$("#grid_handover td:nth-child(1)").hide();
			$("#grid_handover th:nth-child(1)").hide();
			$("#grid_handover td:nth-child(2)").hide();
			$("#grid_handover th:nth-child(2)").hide();
			$("#grid_handover td:nth-child(3)").hide();
			$("#grid_handover th:nth-child(3)").hide();
		...
		if((searchInfo.TYPE == 'day')
			...
			if($("#search_trend_day").is(":checked") || $("#search_trend_hour").is(":checked")) {
				$("#grid_handover th:nth-child(4)").text('일시');
			}
			$("#grid_handover th:nth-child(3)").show();
			$("#grid_handover td:nth-child(3)").show();
			$("#grid_handover th:nth-child(4)").show();
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.handOverInfoLength.length <= 0?0:data.handOverInfoLength[0].TOT_CNT;
...
function excel_download(){
	...
	var columns;
	var i=0;
	$('#grid_handover thead tr').each(function(){
	...
	var url =  "/ps/excelDown?PAGE=INTRAENB&HEADERS="+headers+"&COLUMNS="+columns+
			...
			"&TYPE="+searchInfo.TYPE +"&TCID="+searchInfo.TCID +"&HOCAUSE="+searchInfo.HOCAUSE+"&TITLE=HAND-OVER_INTRA-ENB_통계" + 
			"&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
...
$(document).ready(function(){
	...
	$("#search_trend_day").click(function() {
		//$("#search_system").prop("checked", false);
		$("#search_trend_hour").prop("checked", false);
		if($('#search_handover :radio[name="search_cal"]:checked').val() == 'sum') {
