...
function searchConfig () {
	...
	searchInfo.MSC = $("#search_chk_msc").is(":checked");
...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_epc/ps_mme/ps_mme_ho/sgsap/search',
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.sgsapInfoLength.length <= 0?0:data.sgsapInfoLength[0].TOT_CNT;
...
function excel_download(){
		...
		var url =  "/ps/excelDown?PAGE=SGSAP&HEADERS="+headers+"&COLUMNS="+columns+
				...
				"&TYPE="+searchInfo.TYPE +"&MSC="+searchInfo.MSC+
				"&TITLE=SGsAP_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
