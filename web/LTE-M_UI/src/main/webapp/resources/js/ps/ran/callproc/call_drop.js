...
function changeSortOption(){
	if(!$("#search_chk_cdropcause").is(":checked")){
		var key = $("#grid_estab th:nth-child(8)").attr('id');
...
function searchConfig () {
	...
	searchInfo.CALLDROPESTABCAUSE = $("#search_chk_cdropcause").is(":checked");
...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_ran/ps_ho/ps_calldrop/search',
...
function remakeTd(){
	...
	if($("#search_chk_cdropcause").is(":checked")) {
		...
		$("#grid_estab td:nth-child(8)").show();
		$("#grid_estab th:nth-child(8)").show();
	} else {
		$("#grid_estab td:nth-child(8)").hide();
		$("#grid_estab th:nth-child(8)").hide();
	}
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.callDropInfoLength.length <= 0?0:data.callDropInfoLength[0].TOT_CNT;
...
function excel_download(){
	...
	var url =  "/ps/excelDown?PAGE=CALLDROP&HEADERS="+headers+"&COLUMNS="+columns+
			...
			"&TYPE="+searchInfo.TYPE +"&CALLDROPESTABCAUSE="+searchInfo.CALLDROPESTABCAUSE+
			"&TITLE=CALL_DROP_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
