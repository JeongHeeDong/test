...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_ran/ps_ho/ps_erab/setup_add/search',
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.erabSetupAddInfoLength.length <= 0?0:data.erabSetupAddInfoLength[0].TOT_CNT;
...
function excel_download(){
	...
	var url =  "/ps/excelDown?PAGE=ERABSETUPADD&HEADERS="+headers+"&COLUMNS="+columns+
			...
			"&TYPE="+searchInfo.TYPE +"&TITLE=ERAB-SETUP_ADD_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
