...
function searchConfig () {
	searchInfo.SGW_ID = ("'"+$('#search_du_hidden').val().replace(/,/g, "','")+"'")=="''"?'':("'"+$('#search_du_hidden').val().replace(/,/g, "','")+"'");
	...
	searchInfo.SGW = $("#search_chk_sgw").is(":checked");
...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_epc/ps_sgw/ps_sgw_ho/sgw_bm/search',
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.sgwBmInfoLength.length <= 0?0:data.sgwBmInfoLength[0].TOT_CNT;
...
function excel_download(){
		...
		var url =  "/ps/excelDown?PAGE=SGWBM&HEADERS="+headers+"&COLUMNS="+columns+
				"&SGW_ID="+searchInfo.SGW_ID+"&T_DAY="+searchInfo.T_DAY+"&T_HOUR="+searchInfo.T_HOUR+"&FROM="+searchInfo.FROM+
				"&TO="+searchInfo.TO+"&T_DU="+searchInfo.T_DU+"&EXCEPT_HOUR="+searchInfo.EXCEPT_HOUR +"&EXCEPT_DAY="+searchInfo.EXCEPT_DAY +
				"&TYPE="+searchInfo.TYPE +"&SGW="+searchInfo.SGW+
				"&TITLE=BM_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
...
$(document).ready(function(){
	...
	$('#search_du_btn').click(function () {
		openDialog('#dlg_du', function (fnCloseDlg) {
			...
			sidArr = _.pluck(rows, 'SGW_NAME');
			sids = sidArr.join(",");
			$('#search_du').val(sids);
			sidArr = _.pluck(rows, 'SGW_ID');
			sids = sidArr.join(",");
