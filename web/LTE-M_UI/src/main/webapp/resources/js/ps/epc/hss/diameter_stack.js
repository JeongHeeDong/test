...
function searchConfig () {	
	...
	searchInfo.T_DU =$("#search_system").is(":checked"); //그룹별
...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_epc/ps_hss/diameter_stack/search',
...
function remakeTd(){ //체크박스로 컬럼을 추가하는 동작
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.diameterInfoLength.length <= 0?0:data.diameterInfoLength[0].TOT_CNT;
...
function excel_download(){
	...
	var url =  "/ps/excelDown?PAGE=DIAMETER&HEADERS="+headers+"&COLUMNS="+columns+
			"&T_DAY="+searchInfo.T_DAY+"&T_HOUR="+searchInfo.T_HOUR+"&FROM="+searchInfo.FROM+
			"&TO="+searchInfo.TO+"&T_DU="+searchInfo.T_DU+"&EXCEPT_HOUR="+searchInfo.EXCEPT_HOUR +"&EXCEPT_DAY="+searchInfo.EXCEPT_DAY +
			"&TYPE="+searchInfo.TYPE +"&TITLE=DIAMETER_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
