...
function searchConfig () {
	searchInfo.PGW_ID = ("'"+$('#search_du_hidden').val().replace(/,/g, "','")+"'")=="''"?'':("'"+$('#search_du_hidden').val().replace(/,/g, "','")+"'");
	...
	searchInfo.PGW = $("#search_chk_pgw").is(":checked");
...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_epc/ps_pgw/ps_pgw_ho/pgw_bd/search',
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.pgwBdInfoLength.length <= 0?0:data.pgwBdInfoLength[0].TOT_CNT;
...
function excel_download(){
		...
		var url =  "/ps/excelDown?PAGE=PGWBD&HEADERS="+headers+"&COLUMNS="+columns+
				"&PGW_ID="+searchInfo.PGW_ID+"&T_DAY="+searchInfo.T_DAY+"&T_HOUR="+searchInfo.T_HOUR+"&FROM="+searchInfo.FROM+
				"&TO="+searchInfo.TO+"&T_DU="+searchInfo.T_DU+"&EXCEPT_HOUR="+searchInfo.EXCEPT_HOUR +"&EXCEPT_DAY="+searchInfo.EXCEPT_DAY +
				"&TYPE="+searchInfo.TYPE +"&PGW="+searchInfo.PGW+
				"&TITLE=BD_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
...
$(document).ready(function(){
	...
	$('#search_du_btn').click(function () {
		openDialog('#dlg_du', function (fnCloseDlg) {
			...
			sidArr = _.pluck(rows, 'PGW_NAME');
			sids = sidArr.join(",");
			$('#search_du').val(sids);
			sidArr = _.pluck(rows, 'PGW_ID');
			sids = sidArr.join(",");
