...
function changeSortOption(){
	if(!$("#search_chk_estab").is(":checked")){
...
function searchConfig () {
	...
	searchInfo.ESTABCAUSE = $("#search_chk_estab").is(":checked");
...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_ran/ps_ho/ps_rrc/estab/search',
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.estabInfoLength.length <= 0?0:data.estabInfoLength[0].TOT_CNT;
...
function excel_download(){
	...
	var url =  "/ps/excelDown?PAGE=ESTAB&HEADERS="+headers+"&COLUMNS="+columns+
			...
			"&TYPE="+searchInfo.TYPE +"&ESTABCAUSE="+searchInfo.ESTABCAUSE+"&TITLE=RRC-ESTAB_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
...
$(document).ready(function(){
	...
	$('#search_du_btn').click(function () {
		openDialog('#dlg_du', function (fnCloseDlg) {
			...
			$("#divLine option:eq(0)").prop("selected", true);
			fnCloseDlg();
		});
		//dialog 떳을때
		getDU();
