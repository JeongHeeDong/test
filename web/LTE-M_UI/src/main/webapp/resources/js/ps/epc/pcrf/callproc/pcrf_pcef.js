...
function configure() {
	...
	if(!$("#search_chk_pcef_type").is(":checked")){
...
function searchConfig () {
	...
	searchInfo.PCEF = $("#search_chk_pcef").is(":checked");
	searchInfo.PCEF_TYPE = $("#search_chk_pcef_type").is(":checked");
...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_epc/ps_pcrf/ps_pcrf_pcef/search',
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.pcefInfoLength.length <= 0?0:data.pcefInfoLength[0].TOT_CNT;
...
function excel_download(){
		...
		var url =  "/ps/excelDown?PAGE=PCEF&HEADERS="+headers+"&COLUMNS="+columns+
				...
				"&TYPE="+searchInfo.TYPE +"&PCEF="+searchInfo.PCEF+"&PCEF_TYPE="+searchInfo.PCEF_TYPE+
				"&TITLE=PCEF_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
