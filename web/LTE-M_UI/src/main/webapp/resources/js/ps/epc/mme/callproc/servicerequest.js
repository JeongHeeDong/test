...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_epc/ps_mme/ps_mme_ho/sr/search',
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.srInfoLength.length <= 0?0:data.srInfoLength[0].TOT_CNT;
...
function excel_download(){
		...
		var url =  "/ps/excelDown?PAGE=SR&HEADERS="+headers+"&COLUMNS="+columns+
				...
				"&TITLE=SR_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
