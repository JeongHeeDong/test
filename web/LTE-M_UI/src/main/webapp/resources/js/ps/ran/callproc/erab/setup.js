...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_ran/ps_ho/ps_erab/setup/search',
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.erabSetupInfoLength.length <= 0?0:data.erabSetupInfoLength[0].TOT_CNT;
...
function excel_download(){
	...
	var url =  "/ps/excelDown?PAGE=ERABSETUP&HEADERS="+headers+"&COLUMNS="+columns+
			...
			"&TYPE="+searchInfo.TYPE +"&ESTABCAUSE="+searchInfo.ESTABCAUSE+
			"&TITLE=ERAB-SETUP_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
