...
function searchConfig () {
	searchInfo.MME_ID = ("'"+$('#search_du_hidden').val().replace(/,/g, "','")+"'")=="''"?'':("'"+$('#search_du_hidden').val().replace(/,/g, "','")+"'");
	...
	searchInfo.DU = $("#search_chk_du").is(":checked");
...
function pageSearch() {
	...
	$.ajax({
		type: 'POST',
		url: '/ps/ps_epc/ps_mme/ps_mme_ho/attach/search',
...
function getlength(){
	...
	$.ajax({
		...
		success: function (data) {
			   totalcount = data.attachInfoLength.length <= 0?0:data.attachInfoLength[0].TOT_CNT;
...
function excel_download(){
		...
		var url =  "/ps/excelDown?PAGE=ATTACH&HEADERS="+headers+"&COLUMNS="+columns+
				"&MME_ID="+searchInfo.MME_ID+"&T_DAY="+searchInfo.T_DAY+"&T_HOUR="+searchInfo.T_HOUR+"&FROM="+searchInfo.FROM+
				"&TO="+searchInfo.TO+"&T_DU="+searchInfo.T_DU+"&EXCEPT_HOUR="+searchInfo.EXCEPT_HOUR +"&EXCEPT_DAY="+searchInfo.EXCEPT_DAY +
				"&TYPE="+searchInfo.TYPE+"&DU="+searchInfo.DU+
				"&TITLE=ATTACH_통계&SUB_TITLE=("+ searchInfo.FROM + "~"+ searchInfo.TO +")&PACKAGE_VER="+searchInfo.PACKAGE_VER;
...
$(document).ready(function(){
	...
	$('#search_du_btn').click(function () {
		openDialog('#dlg_du', function (fnCloseDlg) {
			...
			sidArr = _.pluck(rows, 'MME_NAME');
			sids = sidArr.join(",");
			$('#search_du').val(sids);
			sidArr = _.pluck(rows, 'MME_ID');
			sids = sidArr.join(",");
