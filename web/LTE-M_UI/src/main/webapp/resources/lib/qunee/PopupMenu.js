    ...
    getMenuItems: function(graph, dataObj, evt){
      ...
      if (dataObj) {
        ...
        if ( isNode && !(dataObj.image.indexOf("bg_") >= 0) ) {
          ...
          // 스위치: 스위치 포트 상태 팝업
          if(dataObj.image.startsWith("mme_") || dataObj.image.startsWith("pgw_") || dataObj.image.startsWith("sgw_")
              || dataObj.image.startsWith("hss_")|| dataObj.image.startsWith("pcrf_")
              || dataObj.image.startsWith("du_") || dataObj.image.startsWith("ru_") ) {
            items.push({
              text: 'Trend',
          ...
          if( dataObj.image.startsWith("l2_") || dataObj.image.startsWith("l3_") ) {
            items.push({
              text: '스위치',
              action: function(evt, item) {
                nwtDetail.goDialogSwitch(dataObj.equip_type, dataObj.equip_name, dataObj.system_id, dataObj.system_name);
              }
            });
          }
        ...
        if(dataObj.image.startsWith("mme_") || dataObj.image.startsWith("pgw_") || dataObj.image.startsWith("sgw_")
          ...
          items.push({
            text: '성능상세조회',
            action: function (evt, item) {
              nwtDetail.goPeformanceDetail(dataObj.equip_type, dataObj.equip_name, dataObj.system_id, dataObj.system_name);
            }
          });
        }
          ...
          items.push({
            text: '고장상세조회',
            action: function(evt, item){
              nwtDetail.goFailureDetail(dataObj.equip_type, dataObj.equip_name, dataObj.system_id, dataObj.system_name);
            }
          });
        }
        //if ( isNode && !(dataObj.image.indexOf("bg_") >= 0) ) {
